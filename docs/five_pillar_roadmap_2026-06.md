...
- **100+ 부품/4000+ 면 (KR600, RC_Buggy) 의 MONOLITHIC box-mode hausdorff RE 는 본질적 한계 — 타임아웃.** dedup 은 중복 인스턴스에만 유효(148개가 전부 다르면 풀코스트). 2026-06-18 재확인(faulthandler 프로파일): import 만 ~174s + 검출이 비선점(non-preemptible) 스레드풀 안에서 진행 + 결국 OCCT segfault → 900s 워치독이 가리던 하드 스케일 한계. `ThreadPoolExecutor.result()` 안에서 멈추므로 사후 wall-clock budget 으로 끊을 수 없음. `box_complex.json` 에 TIMEOUT 으로 기록(머신-정직). **per-component `assembly_reverse_engineer` 만이 완료 경로지만, 그것은 volume-weighted feature match_ratio 만 산출하고 reconstructed-body hausdorff 는 산출하지 않음 — 절대 재구성-정확도 승리로 보고하지 말 것**(anti-fake-accuracy LAW). `analyze_part` 는 closed_shells≥3 을 flat preserve_brep 로 라우팅하되 실패 시 box 로 fallback 하므로, 이 XL 파일들은 preserve 도 타임아웃 → box fallback(여전히 미완). 즉 line 38-42 의 "대형 어셈블리 KNOWN-GAP 해소" 는 per-component 피처 커버리지 한정이며, **모놀리식 box-hausdorff 재구성은 여전히 KNOWN-GAP**.
