"""swept_boss_along_curve — atomic.

Sweep a 2D profile sketch along an arbitrary 3D polyline or B-spline path,
fusing the resulting tube to the body to create a curved raised feature.

Use cases: cable-guard ridges, raised decorative trim that follows a curve,
ergonomic raised grip lines, surface-mounted snake-shaped bosses.

Args:
    face_selector: SelectorRef       — face where the boss starts (planar ±Z only for v0)
    profile_sketch: SketchSpec       — 2D cross-section (any SketchSpec kind)
    path_points: list of 3D points   — ≥2 world-coord points; first should sit on
                                       (or just above) the chosen face's center
    path_type:    "polyline" | "bspline"
    top_blend_r_mm:  float = 0.0     — fillet edges at the path-end Z plane
    base_blend_r_mm: float = 0.0     — fillet edges where boss meets host face

Mirror of swept_pocket_along_curve, but uses BRepAlgoAPI_Fuse instead of Cut.
"""
...
def _initial_tangent(
    ...
    """Initial tangent direction at points[0]. First-chord approximation
    suffices to orient the profile so the sweep starts cleanly."""
    p0 = points[0]
    p1 = points[1]
...
@skill(
    name="swept_boss_along_curve",
    category="modify/boss",
    level="atomic",
    summary="Sweep a 2D profile along a 3D polyline or B-spline path and "
            "fuse the resulting tube onto the body. Used for raised cable "
            "guards, decorative ridges, ergonomic grip lines, snake-shaped "
            "bosses that follow a free-form curve.",
    selector_kinds=["faces"],
    history_rules={
        "target_face":   HistoryRule.MODIFIED_INHERIT,
        "swept_walls":   HistoryRule.GENERATED_NEW,
        "swept_top":     HistoryRule.GENERATED_NEW,
        "swept_blends":  HistoryRule.GENERATED_NEW,
    },
    preconditions=[
        "pc.face_selector_matches_one",
    ],
    produces_features=["plateau", "swept_boss"],
...
class SweptBossAlongCurve(SkillBase):
    ...
    def _apply(self, body: Any, args: Args) -> SkillResult:
        ...
        if not faces:
            raise RuntimeError(
                f"swept_boss_along_curve: face_selector matched 0 — "
                f"{args.face_selector.model_dump()}"
            )
        target_face = faces[0]
        # Validate planar ±Z face (v0 contract).
        _center, normal = _face_plane_normal(target_face)
        if abs(normal[2]) <= 0.95:
            raise NotImplementedError(
                "swept_boss_along_curve v0: host face must be planar ±Z"
            )
        ...
        # 5. fuse with body
        fuse = BRepAlgoAPI_Fuse(shape, sweep_solid)
        fuse.Build()
        if not fuse.IsDone():
            raise RuntimeError("swept_boss_along_curve fuse failed")
        new_shape = fuse.Shape()

        # 6. optional top blend at path-end Z plane
        if args.top_blend_r_mm > 0:
            end_z = args.path_points[-1][2]
            new_shape, _ = _fillet_edges_near_z(
                new_shape, end_z, args.top_blend_r_mm,
            )
        # 7. optional base blend where boss meets host face
        if args.base_blend_r_mm > 0:
            base_z = args.path_points[0][2]
            new_shape, _ = _fillet_edges_near_z(
